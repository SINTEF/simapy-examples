...
from sima.workflow import WorkflowTask, WorkflowSetNode
import csv
...
def set_json_workflowset_input_file(
    ...
    if not foundStorageTasks:
        raise ValueError(f"Could not find a StorageTask with the name '{storage_task_name}'.")
    if not foundWorkflowTasks:
        raise ValueError(f"Could not find a WorkflowTask with the name '{workflow_task_name}'.")
    if not foundWorkflowSet:
...
def generate_workflowset_input_file(file_out: str, input_variables: dict) -> Path:
    """
    """

    # Shorthand dict notation
    d = input_variables
    
    # Check if length of all inputs are the same
    d_lengths = [len(val) for val in d.values()]
    set_length = d_lengths[0]
    for input_length in d_lengths:
        if set_length != input_length:
            raise ValueError("Values in input_variables does not have the same length")

    with open(file_out, "w", newline="") as f:
        file_header = "'" + " ".join(d.keys()) + "\n"
        f.write(file_header)
        csv.writer(f, delimiter=" ").writerows(zip(*d.values()))
